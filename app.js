/*
 * =================================================================================
 * APP.JS - Tawal Academy Client Logic (FIXED VERSION)
 * Version: 16.0.0 (Complete Fix - All Issues Resolved)
 * =================================================================================
 * التحديثات الرئيسية:
 * ✅ إصلاح نظام المصادقة بالكامل
 * ✅ إصلاح حفظ النتائج مع studentId الصحيح
 * ✅ إصلاح تحميل Dashboard
 * ✅ إصلاح نظام المستويات
...
// استخدام localStorage بدلاً من sessionStorage لتجنب فقدان البيانات
...
async function verifyExistingStudent() {
    ...
    try {
        ...
        if (!response || !response.ok) {
            // الطالب غير موجود، مسح البيانات القديمة
            localStorage.clear();
            return false;
        }
...
/* -------------------------------------------------------------------------- */
/* 6. نقطة الانطلاق الرئيسية (مُصلحة)                                        */
/* -------------------------------------------------------------------------- */
...
async function initIndexPage() {
    ...
    // ✅ تحميل البيانات مع معالجة أخطاء أفضل
...
/* -------------------------------------------------------------------------- */
/* 8. صفحة الاختبار ونظام المستويات (مُصلح)                                   */
/* -------------------------------------------------------------------------- */
...
async function initQuizPage(subjectKey) {
    ...
    LEVEL_CONFIG.forEach((lvl, idx) => {
        ...
        if (idx > 0) {
            ...
            // ✅ مطابقة محسّنة باستخدام اسم المادة ورقم المستوى
            const prevAttempts = pastResults.filter(r => {
                if (!r.quizname) return false;
                
                // التحقق من اسم المادة والمستوى
...
async function initDashboardPage() {
    ...
    try {
        ...
        // ✅ استخدام studentId الصحيح في الـ URL
        ...
        const bySubject = {};
        results.forEach(r => {
            // ✅ استخدام quizname بأحرف صغيرة (PostgreSQL)
        ...
        for (const subjectName in bySubject) {
            ...
            subject.results.forEach(r => {
                // ✅ استخدام أسماء الأعمدة بأحرف صغيرة
...
function logActivity(activityType, subjectName = null) {
    ...
    apiRequest('/log-activity', {
        method: 'POST',
        body: JSON.stringify({ 
            studentId: CURRENT_STUDENT_ID, // ✅ إضافة studentId
            activityType, 
            subjectName 
        })
...
async function saveQuizResult(quizName, score, totalQuestions, correctAnswers, subjectId) {
    ...
    try {
        await apiRequest('/quiz-results', {
            method: 'POST',
            body: JSON.stringify({
                studentId: CURRENT_STUDENT_ID, // ✅ استخدام studentId الصحيح
...
/* -------------------------------------------------------------------------- */
/* 5. نظام المصادقة والحماية (مُصلح بالكامل)                                  */
/* -------------------------------------------------------------------------- */
